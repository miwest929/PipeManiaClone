...
var PipeGrid = /** @class */ (function () {
    ...
    PipeGrid.prototype.renderOozeCell = function (ctx, x, y, tileId, progress) {
        ...
        var startIdx = 0;
        var deltaIdx = 1;
        var shouldEndFn = function (idx) { return idx <= progress; };
        if (this.oozeReverse) {
            startIdx = progress;
            deltaIdx = -1;
            shouldEndFn = function (idx) { return idx >= 0; };
        }
        for (var i = startIdx; shouldEndFn(i); i += deltaIdx) {
            var progressLine = oozeProgress[i];
            if (progressLine) {
...
var nextTileView = new NextTileComponent();
